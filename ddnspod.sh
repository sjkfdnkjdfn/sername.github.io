  ...
  'Linux')
	...
	case $IPtype in
		...
		then
			...
			wget --no-check-certificate --secure-protocol=TLSv1_2 --quiet --output-document=- "http://ip.3322.net/" | grep -E -o '([0-9]+\.){3}[0-9]+' | head -n1 | cut -d' ' -f1
		...
		curl -k -s "http://ip.3322.net/" | grep -E -o '([0-9]+\.){3}[0-9]+' | head -n1 | cut -d' ' -f1
		fi
		;;
