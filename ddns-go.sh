  ...
  'Linux')
	...
	case $IPtype in
		...
		then
			...
			wget --no-check-certificate --secure-protocol=TLSv1_2 --quiet --output-document=- "https://icanhazip.com/" | grep -E -o '([0-9]+\.){3}[0-9]+' | head -n1 | cut -d' ' -f1

		else
		curl -k -s "https://icanhazip.com/" | grep -E -o '([0-9]+\.){3}[0-9]+' | head -n1 | cut -d' ' -f1
		fi
		;;
